...
`global.css` has two reflow breakpoints on top of the desktop 3-pane grid. At **≤1750px** the grid collapses to a stacked, scrolling two-column layout. At **≤640px** `dashboard.tsx` swaps the whole layout for `MobileDashboard` — a slim app bar (compact wordmark, a Filters button badged with the active filter count, an info button), an area-summary strip, a full-bleed `MapView`, and a bottom sheet. The **area strip** (`MobileAreaStrip`, pinned under the app bar) carries the map-scoped info: restaurant count and a compact stacked grade bar; tapping it slides a drawer down over the map with the `GradeChart` donut and per-grade tally. The area-strip drawer and the two app-bar drawers (Filters, Info) are mutually exclusive — one `activeDrawer` state in `MobileDashboard` controls all three. Each drops from below the strip as a full-bleed `--bg-panel` panel with a bordered card inset `0.5rem`; the Info drawer is two such panels (scrolling meta/guide + pinned footer) with a `0.5rem` map gap between. The **bottom sheet** is purely restaurant info, with two detents (`useBottomSheet`): `peek` shows the selected restaurant's list card (or a browse prompt) over a full-size map; `open` shows search, the tab list, and either the paginated restaurant list or the selected restaurant's details/report. Tapping a **map dot** goes to `peek` (a glance, map still full-size); tapping a **list row** or the peek card `open`s the sheet. Every tab fills a bounded box with the panel header fixed: List paginates; Details and Report keep their header in place while `.panel-scroll-content` scrolls. On Details that scroll box sits above the score-history chart, which is pinned to the bottom of the sheet (Details tab only, like the Info drawer's footer). Tapping a dot on that pinned chart highlights the matching Inspection History row and scrolls it into view without leaving the Details tab (`previewInspection` in `selectionReducer.ts`); the point tooltip drops to a compact grade + score label on the active dot (`tooltipVariant="compact"`) since the full date + badges card is too large for the short chart. The **Info drawer** is the single info surface on phones (`MobileInfoContent`): a compact dataset-meta block, then the desktop Dashboard Information sections (overview, interaction, grade/status definitions, data attribution, data notes, health resources) plus the map legend — the shared blocks live in `InfoPopupSharedContent.tsx`, so `DashboardGuide` (desktop), `MapLegendInfoContent` (map popup), and the mobile drawer all compose from the same source. The map's own corner info button is hidden at this width. The attribution footer sits below as a detached panel with a 0.5rem gap. `GradeChart` and `PerformanceChart` only mount when their drawer / detent opens, keeping Recharts off the phone critical path. All shared state stays in `dashboard.tsx` and is passed down.
...
- The mobile bottom sheet toggles between its two detents (`peek` / `open`) by tapping the grab handle; a drag gesture is not wired up yet
