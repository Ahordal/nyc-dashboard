...
- **Grade Breakdown Chart:** Interactive Recharts-powered donut chart providing a visual proportional breakdown of graded restaurants within the active map view (scoped to the selected grades when a grade filter is on, so it matches the map and list).
...
- **Search Radius:** Top-right crosshair tool for scoping to a walkable area instead of the map viewport. Activate it, click the map to drop a center point, and concentric 0.25 / 0.50 / 1 mi rings are drawn as a `GraphicsLayer` (so the restaurant points are never spatially filtered — every dot stays visible); a segmented picker switches the active ring and the view auto-frames so the whole circle is visible. While a point is set, the Restaurant List, KPI panel, and Grade Breakdown chart re-scope from "what's in view" to "what's inside the active ring" — computed client-side with a haversine distance against the list the sidebar already holds in memory — and map pan/zoom stops re-running the viewport query. The list gains a **Distance** column and sort field (closest-first, auto-selected while the radius is active, reverting to the previous sort field when the point is cleared), and the KPI panel's scope label switches from "in map view" to "within 0.25 mi".
- **Locate Me (mobile):** A location-arrow chip below the Search Radius button, shown only in the phone layout. Tapping it makes a one-shot `navigator.geolocation` request (`useGeolocation`) — not a live watch — with the spinner running inside the button through the permission prompt and the fix. A successful fix draws a larger blue dot plus a translucent accuracy circle on a dedicated `GraphicsLayer` above the restaurant points and reframes the map: with a restaurant selected it fits both your position and that restaurant into the slice of map left visible above the bottom sheet (padded to the sheet's live height); with nothing selected, or when the two points sit within ~150 m, it just recenters on the fix. Decoupled from the Search Radius tool, it also adds a **Distance** line to every Restaurant List card measured from your position, and unlocks the **Distance** sort field (closest-first), exactly as a Search Radius point does. No list re-scoping and no URL change, though; a Search Radius point takes over the distance readout and sort when one is set, and losing both origins reverts a Distance sort to the previously chosen field. Once located, a `MY LOCATION` label + clear (×) appears to the left of the button (mirroring the Search Radius active panel); the arrow itself then re-locates on tap, and the × drops the dot and the distances together. A fix outside the loose NYC bounding box (`shared/nycBounds.mjs`, the same box the pipeline uses for coordinate sanity checks) draws nothing and shows a brief uppercase "Your location is outside NYC" notice instead. Blocked, unavailable, or insecure-context outcomes leave the button muted with the reason in its tooltip. The coordinate is never stored, put in the URL, or sent anywhere — it lives in component state for the session. Separate from the Search Radius tool, which stays a manual map tap so it can be placed at any location.
...
- **Shareable Views:** Active grade/borough filters, search query, the selected restaurant (`?camis=`), and a placed Search Radius (centre + distance, as `?radius=<lat>,<lng>,<miles>`) are all synced to the URL, so a link can be copied or bookmarked straight back to that exact view — reloading re-drops the radius pin, redraws the rings, and re-frames the map. The `?camis=` restaurant is resolved with a direct layer query, so the link still lands even when the target starts off-screen or is hidden by the active grade filter, and the map flies to it.
...
A single `ErrorBoundary` component (`src/components/ErrorBoundary.tsx`) wraps the app root in `main.tsx` and each lazy panel's `<Suspense>` in `dashboard.tsx`. A render-time error — including a lazy chunk that fails to download — shows an inline `ErrorFallback` with a reload prompt in just that panel, rather than unmounting the whole dashboard. The `PerformanceChart` boundary is keyed to the selected restaurant so switching restaurants clears a stale error on its own. `ErrorBoundary` only catches render-time throws, so `MapView` handles its own async load failures (a 404 on `latest-inspections.geojson`, or a missing/invalid/over-quota ArcGIS key) separately: it catches the `view.when()` / `layer.load()` rejection and covers the canvas with the same `ErrorFallback` UI, whose **Retry** button rebuilds the view in place without a full reload.

### Responsive layout

`global.css` has two reflow breakpoints on top of the desktop 3-pane grid. At **≤1750px** the grid collapses to a stacked, scrolling two-column layout. At **≤640px** `dashboard.tsx` swaps the whole layout for `MobileDashboard` — a slim app bar (compact wordmark, a Search button, a Filters button badged with the active filter count, an info button), an area-summary strip, a full-bleed `MapView`, and a bottom sheet. The **area strip** (`MobileAreaStrip`, pinned under the app bar) carries the map-scoped info: restaurant count and a compact stacked grade bar; tapping it slides a drawer down over the map with the `GradeChart` donut and per-grade tally. The area-strip drawer and the three app-bar drawers (Search, Filters, Info) are mutually exclusive — one `activeDrawer` state in `MobileDashboard` controls all four. Each drops from below the strip as a full-bleed `--bg-panel` panel with a bordered card inset `0.5rem`; the Info drawer is two such panels (scrolling meta/guide + pinned footer) with a `0.5rem` map gap between. The **Search** drawer holds the reused desktop `ExplorerSearch`; opening it forces the List tab and raises the sheet to its `half` detent so the generated list stays visible over a live-filtering map. From `half` the down chevron drops the sheet to `peek` (freeing the map to pan) or the up chevron goes to `open`; either way the query stays live. Closing the drawer keeps the current detent while a query is still committed (the filtered list shouldn't vanish with the input) and only forces `peek` once the search is empty; raising the sheet to `open` closes the drawer. While the drawer is closed with a live query the Search button carries an accent dot. The **bottom sheet** is purely restaurant info, with three detents (`useBottomSheet`): `peek` shows the selected restaurant's list card (or a browse prompt) over a full-size map; `half` shows the restaurant list, or the selected restaurant's Details pane, over a still-visible map that has panned to the pin (Search mode and the first stop after a selection); `open` is the full-height explorer. Tapping a **map dot** goes to `peek` (a glance, map still full-size); tapping a **list row** selects the restaurant and lands at `half` on its Details, while tapping the **peek card** (a committed selection) jumps straight to `open`. Detent stepping is two centred chevrons in the handle row: an up chevron (`peek` → `half` → `open`) and a down chevron (`open` → `half` → `peek`) — `peek` shows only up, `open` only down, `half` both side by side. Every tab's content flows in the one sheet scroll (`.mobile-sheet-scroll-body`) below a fixed tab strip. The List tab shows a fixed 10 restaurants per page (`PAGE_SIZE` in `RestaurantList.tsx`, the same count on desktop); its header, sort bar, cards and pagination row scroll together as a block at both detents. At `open`, Details and Report keep their header in place while `.panel-scroll-content` scrolls, and on Details that scroll box sits above the score-history chart pinned to the bottom of the sheet (like the Info drawer's footer). At `half` the Details/Report pane is unbound — record, inspection history and score-history chart all flow together in the one sheet scroll; the chart only pins once `open` gives the history list room to scroll under it. Tapping a dot on the score chart highlights the matching Inspection History row and scrolls it into view without leaving the Details tab (`previewInspection` in `selectionReducer.ts`); the point tooltip drops to a compact grade + score label on the active dot (`tooltipVariant="compact"`) since the full date + badges card is too large for the short chart. The **Info drawer** is the single info surface on phones (`MobileInfoContent`): a compact dataset-meta block, then the desktop Dashboard Information sections (overview, interaction, grade/status definitions, data attribution, data notes, health resources) plus the map legend — the shared blocks live in `InfoPopupSharedContent.tsx`, so `DashboardGuide` (desktop), `MapLegendInfoContent` (map popup), and the mobile drawer all compose from the same source. The map's own corner info button is hidden at this width. The attribution footer sits below as a detached panel with a 0.5rem gap. `GradeChart` and `PerformanceChart` only mount when their drawer / detent opens, keeping Recharts off the phone critical path. All shared state stays in `dashboard.tsx` and is passed down.
...
| `src/utils/userLocationGraphics.test.ts` | Verifies the "locate me" graphics builder: accuracy circle then dot in draw order, dot at the fix and larger than a score dot, circle radius equal to the accuracy in metres, and no circle when accuracy is zero |
| `src/hooks/useGeolocation.test.ts` | Verifies the one-shot geolocation hook: the locating→success transition and fix shape, each `GeolocationPositionError` code mapping to its `errorKind`, the insecure-context and no-API short-circuits (no `getCurrentPosition` call), the `maximumAge` 30s→0 flip on the second request, `clearLocation` resetting to idle (and restoring the fast cached path), and a stale callback being ignored after a newer request or a clear |
...
- Map hover cards and name labels rely on mouse hover, with no touch equivalent yet; on phones (≤640px) the dashboard switches to a dedicated bottom-sheet layout, but tapping a map dot is the only way to preview a restaurant there
- The mobile bottom sheet has three detents (`peek` / `half` / `open`): selecting a restaurant from the list or opening the app-bar Search drawer lands at `half`, tapping the peek card jumps to `open`; two centred chevrons in the handle row step it one detent at a time (up / down; `half` shows both). A drag gesture is not wired up.
